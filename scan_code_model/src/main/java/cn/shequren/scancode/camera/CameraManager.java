        ...
        if (mCamera == null) {
            try {
                ...
                if (mCamera != null) {
                    ...
                    ScanCodeMode scanCodeMode = ScanCodeMangerUtils.newInstance().mScanCodeMode;
                    Camera.Parameters mParameters = mCamera.getParameters();
                    if (scanCodeMode != null && scanCodeMode == ScanCodeMode.BAR_CODE) {
                        if (mParameters.isZoomSupported()) {
                            // 设置成最大倍数的1/10，基本符合远近需求
                            mParameters.setZoom(mParameters.getMaxZoom() / 8);
                        }
                    }
